...
class LambdaControllerRegistrar:
    """
    ...
    def register(self, routes: dict, ctx) -> None:
        ...
        :param routes: dict to populate with routeKey → {'handler': callable} entries
        ...
        for name, comp in components.items():
            ...
            if declared_routes is not None and isinstance(declared_routes, list):
                for route in declared_routes:
                    ...
                    # Convert :param style to {param} for API Gateway compatibility
                    lambda_path = re.sub(r":([a-zA-Z_]\w*)", r"{\1}", path)
                    route_key = f"{method.upper()} {lambda_path}"
